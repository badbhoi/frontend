...
//     const response = await api.get(`/rest/v1/profiles?id=eq.${userId}&select=*`)
//     return response.data[0]
//   },

//   async getAllUsers() {
//     const response = await api.get('/rest/v1/profiles?select=*&order=created_at.desc')
...
//     const response = await api.delete(`/rest/v1/profiles?id=eq.${userId}`)
//     return response.data
//   },
...
//     const response = await api.get('/rest/v1/profiles?select=balance')
//     const data = response.data
    
//     const totalUsers = data.length
//     const totalBalance = data.reduce((sum: number, user: any) => sum + (user.balance || 0), 0)
